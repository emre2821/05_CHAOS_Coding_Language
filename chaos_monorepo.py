...
class ChaoParser:
    ...
    def parse_structured_core(self) -> Node:
        ...
        while not self.is_at_end():
            ...
            if not self.check(TokenType.IDENTIFIER):
                # Not a key-value tag -> rewind and hand off to next layer
                self.current -= 1
                break
    ...
    def _parse_tag_triplet(self) -> Optional[Dict[str, Any]]:
        ...
        Returns dict {tag, kind, value?} or None if not a recognized pattern.
        """
        if not self.check(TokenType.LEFT_BRACKET): return None
        # probe ahead without committing
        save = self.current
        self.advance()  # [
        if not self.check(TokenType.IDENTIFIER):
            self.current = save; return None
        tag = self.advance().value  # EMOTION/SYMBOL/RELATIONSHIP/other
        if not self.match(TokenType.COLON):
            # not a triplet; restore and let other layers try
            self.current = save; return None
        if not self.check(TokenType.IDENTIFIER):
            self.current = save; return None
        kind = self.advance().value

        # optional ":value" which can be IDENTIFIER or NUMBER
        val_tok = None
        if self.match(TokenType.COLON):
            if self.check(TokenType.IDENTIFIER) or self.check(TokenType.NUMBER):
                val_tok = self.advance()
            else:
                # tolerate weirdness: leave val_tok=None and continue
                pass

        self.consume(TokenType.RIGHT_BRACKET, "] after tag")
        return {"tag": tag, "kind": kind, "value": (val_tok.value if val_tok else None), "value_type": (val_tok.type.name if val_tok else None)}

    def parse_emotive_layer(self) -> Node:
        emotions: List[Dict[str, Any]] = []
        # Also collect non-emotion tags to avoid silent data loss (non-breaking)
        ...
        while not self.is_at_end():
            ...
            if tag == "EMOTION":
                # intensity can be NUMBER or IDENTIFIER; coerce robustly
                try:
                    iv = int(val) if (vtype == "NUMBER") else int(str(val)) if val is not None else 5
                except Exception:
                    iv = 5
                iv = max(0, min(iv, 10))
                emotions.append({"name": kind.upper(), "intensity": iv})
            else:
                # Preserve for future (agent could use via env["emotive_extras"])
                extras.append({"tag": tag, "kind": kind, "value": val})

        # Non-breaking API: only emotions in value; extras are accessible if needed
        return Node(NodeType.EMOTIVE_LAYER, value=emotions if emotions else [])
    ...
    def parse_chaosfield_layer(self) -> Node:
        ...
        while not self.is_at_end() and not self.check(TokenType.RIGHT_BRACE):
            tok = self.advance()
            # Keep strings as-is; others as their textual value
            if tok.type == TokenType.STRING:
                parts.append(tok.value)
            elif tok.value is not None:
                parts.append(str(tok.value))
        if self.check(TokenType.RIGHT_BRACE): self.advance()
        return Node(NodeType.CHAOSFIELD_LAYER, value=(" ".join(p for p in parts if p)).strip())
